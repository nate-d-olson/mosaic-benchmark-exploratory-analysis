...
snvs_af_df <- snvs_df %>%
    ...
    mutate(AltCounts = if_else(TQSS_NT == 1, tier1AltCounts, tier2AltCounts),
           ...
           AF = as.numeric(AltCounts) / (as.numeric(AltCounts) + as.numeric(RefCounts)))
```
...
PICK-UP HERE - want plot with all variants and thoes being validated
...
bind_rows(pass_candidate_vars, non_pass_candidate_vars) %>% 
    ...
    arrange(CHROM, POS) %>% 
    write_tsv(here("data","putative_mosaic_variants.txt"))
```
...
Ony including PASSing indels in the list of variants to validate
```{r}
indels_df %>% filter(FILTER == "PASS")
```
```{r}
indels_df %>% 
    filter(FILTER == "PASS") %>% 
    select(CHROM, POS, FILTER, SomaticEVS) %>% 
    arrange(CHROM, POS) %>% 
    write_tsv(here("data","candidate_mosaic_indels.txt"))
```
...
```{r}
putative_bed <- putative_var_df %>% 
    ungroup() %>% 
    select(CHROM, POS) %>% 
    mutate(START = POS - 50, END = POS + 50) %>% 
    select(-POS)

write_tsv(putative_bed, file = here("putative_var.bed"), col_names = FALSE)
```

Excluding putative variants from the merged variant list
```
subtractBed -header -a merged-variants.vcf.gz -b putative_var.bed > merged-variants_no-putatitive.vcf
```

### Candidate Control Variant List
```{r}
merged_bench_vcf_df <- read_tsv(here("merged-variants_no-putatitive.vcf"),
                   comment = "##") %>% 
    rename(CHROM = `#CHROM`) %>% 
...
control_var_df %>% 
    top_n(500, wt = -var_dist) %>% 
    arrange(CHROM, POS, ID, var_dist) %>% 
    write_tsv(here("data","control_variants.txt"))
```
